...
def test_features_are_deterministic_and_invalid_weight_is_missing():
    frame = pd.DataFrame({
        ...
        "date": ["2025-12-01"], "distance": [360.0], "weight": [-1.0],
        "pickup_lat": [38.0], "pickup_lon": [-84.5], "delivery_lat": [41.0], "delivery_lon": [-85.1],
...
def test_december_inputs_can_omit_operational_quote_fields():
    frame = pd.DataFrame({
        "pickup": ["Lexington"], "delivery": ["Fort Wayne"], "equipment": ["Dry Van"],
        "date": ["2025-12-01"], "distance": [360.0], "weight": [32000.0],
    })
    result = make_model_features(frame)
...
def test_holiday_features_flag_christmas_week():
    frame = pd.DataFrame({
        "pickup": ["Lexington"], "delivery": ["Fort Wayne"], "equipment": ["Dry Van"],
        "date": ["2025-12-25"], "distance": [360.0], "weight": [32000.0],
    })
    result = make_model_features(frame)
